...
<div class="page-container">
  <form class="form form-horizontal" id="form-admin-add">
		...
		<div class="formControls col-xs-8 col-sm-9">
			<span class="select-box" style="width: 150px;"> <select id="selectPermissionType"
				class="select" name="type" size="1">
					<option value="1" <c:if test="${entity.type == 1}">selected="selected"</c:if>>功能</option>
					<option value="3" <c:if test="${entity.type == 3}">selected="selected"</c:if>>页面</option>
					<option value="2" <c:if test="${entity.type == 2}">selected="selected"</c:if>>菜单</option>
			</select>
			</span>
		</div>
	</div>
		 ...
		 <div class="formControls col-xs-8 col-sm-9">
			<span class="select-box" style="width: 150px;"> 
			 <select id="selectTopmenu" class="select" name="parentId" size="1">
			   <c:if test="${not empty entity.parentId }"><option value="${entity.parentId }" selected="selected">${entity.parentName}</option></c:if>
	    ...
	    <div id="icon-content" style="display:none;">
		 ...
		 <div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-2">
				<input type="text" class="input-text" value="${entity.sort}" placeholder="请输入菜单排序" id="sort" name="sort">
			</div>
	  </div>
	</div>
		...
		<div class="formControls col-xs-8 col-sm-9">
			<span class="select-box" style="width: 150px;"> 
			 <select id="selectSubmenu" class="select" name="menuId" size="1">
			   <c:if test="${not empty entity.menuId}"><option value="${entity.menuId}">${entity.menuName}</option></c:if>
			 </select>
			</span>
		</div>
	 </div>
	...
	<div class="row cl">
		...
		<div class="formControls col-xs-8 col-sm-9">
			<span class="select-box" style="width: 150px;"> <select id="selectIsRoot"
				class="select" name="isRoot" size="1">
					<option value="0" <c:if test="${entity.isRoot == 0}">selected="selected"</c:if> >否</option>
					<option value="1" <c:if test="${entity.isRoot == 1}">selected="selected"</c:if>>是</option>
			</select>
			</span>
		</div>
	</div>
	 ...
	 <div class="row cl">
		...
		<div class="formControls col-xs-8 col-sm-9">
			<span class="select-box" style="width: 150px;"> <select id="selectPermissionState"
	 ...
	 <div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="${entity.name}" placeholder="请输入资源名称" id="name" name="name">
			</div>
	  </div>
	  ...
	  <div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="${entity.url}" placeholder="请输入资源访问地址，顶级菜单时此处留空!" id="url" name="url">
			</div>
	  </div>
	  ...
	  <div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="${entity.code}" placeholder="请输入资源编码" id="code" name="code">
			</div>
	  </div>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<textarea name="note" cols="" rows="" class="textarea"  placeholder="权限备注...500个字符以内" dragonfly="true" onKeyUp="$.Huitextarealength(this,500)">${entity.note}</textarea>
...
$(function(){
	...
	$("#form-admin-add").validate({
		...
		submitHandler:function(form){
			...
			$(form).ajaxSubmit({
				type: 'post',
				url: "${pageContext.request.contextPath}/web/permission/update" ,
                success: function(data){
                    if(data.success){
                        showSuccess("修改成功!");
