package com.anmong.common.fastsql.dao;
...
import com.anmong.common.fastsql.SQL;
import com.anmong.common.fastsql.SQLFactory;
import com.anmong.common.fastsql.dto.BatchUpdateResult;
import com.anmong.common.fastsql.dto.ResultPage;
import com.anmong.common.fastsql.util.EntityRefelectUtils;
import com.anmong.common.fastsql.util.StringExtUtils;
...
import static com.anmong.common.fastsql.util.StringExtUtils.camelToUnderline;
...
public abstract class BaseDAO<E, ID> {
    ...
    public int insert(E entity) {
        ...
        //如果id没有值，则通过自动生成uuid
        if (StringUtils.isEmpty(EntityRefelectUtils.getFieldValue(entity, idField))) {
            EntityRefelectUtils.setFieldValue(entity, idField, UUID.randomUUID().toString().replace("-", ""));
        }
        /*Field id = EntityRefelectUtils.getIdField(entity);
        ...
        if (StringUtils.isEmpty(val)) {
             String type = id.getType().toString();//得到此属性的类型  
...
*/
    ...
    public int updateSelective(E entity) {
        ...
        final StringBuilder sqlBuilder = new StringBuilder();
        fieldsWithoutId.forEach(field ->{
            if(EntityRefelectUtils.getFieldValue(entity, field) != null && !"".equals(EntityRefelectUtils.getFieldValue(entity, field))){
                sqlBuilder.append("," + StringExtUtils.camelToUnderline(field.getName()) + "=:" + field.getName());
            }
        }
        );
    ...
    public int updateSetWhere(String set, String where, Object... params) {
        return getSQL().useSql("UPDATE " + tableName + " SET " + set + " WHERE " + where).varParameter(params).update();
    }

    public int updateSetWhereById(String set, String id, Object... params) {
        return getSQL().useSql("UPDATE " + tableName + " SET " + set + " WHERE id = '" + id+"'").varParameter(params).update();
    }


    public int updateSet(String set, Object... params) {
    ...
    public E selectNotNullOneById(ID id) {
        E returnObject;
        try {
            returnObject = getSQL()
                    .SELECT("*")
        ...
        if (returnObject == null){
            throw CommonException.businessException("未找到该信息");
        }
        return returnObject;
    }

    public E selectNotNullOneById(ID id,String message) {
        E returnObject;
        try {
            returnObject = getSQL()
                    .SELECT("*")
        ...
        if (returnObject == null){
            throw CommonException.businessException(message);
        }
        return returnObject;
    }
